	...
	HestonStochasticVolalityParameters^% volParams)
{
	...
	double rho_probabilities[3] = { 0.25, 0.5, 0.25 };
	double rho_choice[3] = { -0.5, -0.7, -0.9 };
	std::default_random_engine generator;
	std::discrete_distribution<int> distribution{ 0.25, 0.5, 0.25 };	
	...
	List<double>^ rhos = gcnew List<double>();
	int callCounter = 0;
	int putCounter = 0;
	...
	{				
		...
		{												
			...
			double rho_i = rho_choice[weightedProbabilityIndex];			
			double dw_s = rho_i * dw_v + Math::Sqrt(1 - Math::Pow(rho, 2)) * dw_i; 
			...
			spotGraph[i]->Add(S);
			rhos->Add(rho_i);
			if (Math::Max((double)S - (double)K, 0.0) != 0) callCounter++;
			if (Math::Max((double)K - (double)S, 0.0) != 0) putCounter++;
		}		
	}	
	double call = Math::Exp(-r * T) * payoff_sum / totalSimulations;
	double put = Math::Exp(-r * T) * payoff_sumPut / totalSimulations;

	Debug debug = Debug(callCounter, putCounter, rhos, spotGraph, totalSimulations);	
